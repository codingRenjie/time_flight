...
2. **时间池可视化**  
   120 分钟为固定「燃料」。添加学习块 = 挤占自由飞；提前进港 = 省下的整分钟**按顺序逐项补给**后续待飞航段（与超时扣减对称）；**超时 = 后续待飞航段预算联动扣减**（水果经停除外），让孩子看见「用掉就没了」。自由飞恒为执飞顺序最后一程，时间变动的净效应最终体现在自由飞上。
...
- 默认水果经停时长为**刚性接收方**：不参与超时联动扣减，也不接收其他航段提前进港释放的时间。
- 水果经停**自身提前进港**时，释放的整分钟仍按规则 5 补给后续待执飞项（与学习块相同）。
- 孩子可在制定航程时**调整水果经停在列表中的顺序**（拖拽排序），也可改当晚计划分钟数；**不可从任务池删除该模块，也不可移出航程**。
- 起飞时水果经停按**当晚计划时长**执飞（默认 15 分钟），不受前面航段超时或提前影响。
- **水果经停超时**（超过本段计划仍未点「续飞」）时，同样启动对**后续待执飞项**的联动扣减，规则与规则 4 相同；水果经停自身时长不被扣减。
...
- 若当前航段提前进港且仍有未用完的延长分钟，剩余部分按规则 5 按顺序补给后续待执飞项。
...
- 任意航段（自由飞除外，**含水果经停**）若**提前进港**，释放分钟数 = `本段可用预算 − 实际用时`（整分钟，四舍五入）。
- 释放的时间补给当前航段之后所有**待执飞**项（规则 4 同一参与集合：含自由飞，不含水果经停）。
- **补给方式**与规则 4 对称：最小单位 **1 分钟**。每释放 1 分钟，按后续待飞**顺序**给 **1 项 +1 分钟**（轮转）。两项时：第 1 分钟给第 1 项，第 2 分钟给第 2 项；仅一项时两分钟都补给该项。
...
| 提前进港释放 | 不接收；自身提前结束会释放给后续 | 参与，按顺序逐项各 +1 分钟 | 参与，按顺序逐项各 +1 分钟 |
| 延长航程 +10min | 不可用 | **当前执飞航段** +10 | 不变（除非后续联动/提前补给） |
| 顺序 | 孩子可调 | 孩子可调 | 恒为最后一程 |
...
吃水果计划 3 分钟，实际 1 分钟进港，释放 2 分钟；后续待飞：英语朗读 + 自由飞（2 项）→ 朗读 +1、自由飞 +1。

数学计划 45 分钟，实际 40 分钟进港，释放 5 分钟；后续待飞：朗读 + 订正 + 自由飞（3 项）→ 朗读 +2、订正 +2、自由飞 +1（第 1/4 分钟给朗读，第 2/5 分钟给订正，第 3 分钟给自由飞）。

### 示例 C · 延长航程后提前进港（规则 3 + 规则 5）

数学执飞中计划 45 分钟，使用「延长航程 +10」后可用预算 55 分钟；实际 48 分钟进港 → 释放 7 分钟。后续待飞：朗读 + 订正 + 自由飞（3 项）→ 朗读 +3、订正 +2、自由飞 +2。**延长分钟未用完部分与普通提前释放相同，均按规则 5 按顺序补给。**

### 边界确认（v0.3，已对齐）

| 编号 | 场景 | 结论 |
|------|------|------|
| A | 延长后进港，仍有未用完分钟 | 按规则 5 按顺序补给后续所有待执飞项（含自由飞，不含水果经停） |
| B | 延长 10 分钟加在当前执飞段 | 先计入当前段；仅通过联动扣减/提前补给间接影响自由飞 |
...
**屏幕**：两步排航程。`/evening` 任务池定今晚飞哪些与时长；`/evening/order` 只调顺序后起飞。

**首屏信息**：
- 大标题：「今晚总燃料：120 分钟」
- 预计自由飞（溢出时显示超出分钟，禁止进入下一步）
- 副文案：「这 120 分钟里，你来排航班。」

**系统预置（默认进入今晚，且不可移出、不可删除模块；家长 weekly 可改时长）**：
...
**孩子操作**：
1. `/evening`：在执飞任务池加入/移出学习块、改时长、新建或删除可删模块；英语朗读与水果经停带绿色勾且显示「不可移除」
2. 溢出时先调整，再点「确认任务清单」进入 `/evening/order`
3. `/evening/order`：拖拽排序全部航段；可返回上一页改学习块
4. 点击「开始执飞」
...
- **后续航段预算条**（超时按顺序逐项扣减时同步缩短；只显示实际可用时间，如「朗读 12 分钟」「自由飞 27 分钟」）

**孩子操作**：
- 点击「本段进港」（提前或到点均可）
- 可选「未完成」：给当前执飞任务打标签（可再点取消），与是否提前/超时无关
- 可选「延长航程 10 分钟」（**整晚限 1 次，仅执飞中、非水果经停**）；**不因超时而强制打断当前学习**
- 底部小字「取消当前航班」：结束当晚全部航程，不统计，页面仅显示「今日航班取消」

**系统行为**：
- 到本段预算用尽：界面轻提示「本段计划时间到了」，**不弹窗、不阻断**；音效/进港动画见 P2
- **提前进港**：释放的整分钟**按顺序补给**后续待飞航段（含自由飞，不含水果经停；水果经停自身提前结束也会释放）；过渡页提示「提前进港 +N 分钟已按顺序补给后续待飞航段」
- 点「未完成」只打标签，不代替进港；日结用实际用时 / 当初计划 对照是否完成
...
> 若超时联动扣减已使某航段预算为 0，该航段已为 `incomplete`，**不再出现在优先级选择中**。
...
**触发条件**（MVP 当前）：所有已排航段（含 mandatory、break、study）均已进港。

> `freeFlyForceStart`（默认 21:10 且 mandatory 已完成则强制进入）为预留字段，尚未接线。
...
**日结内容（约 30 秒）**：
- 动画：今日航线闭合
- **今晚任务回顾**：每项执飞任务列出 **实际分钟 / 当初计划分钟**、**完成 / 未完成**（执飞中点「未完成」打的标签会体现；超时耗尽未执飞项同样记未完成）
- 中性摘要：完成项数 / 必做朗读是否 ✓ / 提前进港补给
- 明早队列：N 项（若有）
- 表情反馈（可跳过）：😊 / 😐 / 😫
...
| 场景 | 系统响应 | 家长应避免 |
|------|----------|------------|
| 数学块超时仍不点进港 | 提醒后继续；**按顺序逐项扣减**后续航段预算；预算归零项标 incomplete | 21:00 后长篇批评 |
| 数学进港后不够做朗读+订正 | **一次**优先级选择；第二项时间不足则中性预告 incomplete | 替孩子决定先做哪科 |
| 联动扣减已使某航段为 0 | 标 incomplete，跳过执飞，进明早队列 | 强制补做已耗尽项 |
...
| 类型 | 代码 | 可删除 | 时长 | 谁设定 | 示例 |
|------|------|--------|------|--------|------|
| 必做 | `mandatory` | 否（模块与航程均不可移除） | 默认可改 | 家长模板 | 英语朗读 15min |
| 经停 | `break` | 否（模块与航程均不可移除） | 默认可改；刚性=不接收联动/补给，自身提前结束会释放 | 家长模板 | 水果 15min |
| 学习 | `study` | 是 | 可调 | 孩子从任务池选 | 数学巩固、订正 |
...
| F-05 | 进港 | 到点/提前进港，进入下一航段 | 提前进港按顺序补给后续待飞航段（含水果经停提前结束） |
...
| F-10 | 自由飞行 | 全部已排学习航段进港后进入 | 显示剩余至 21:30；`freeFlyForceStart` 强制进入待实现 |
| F-11 | 日结进港 | 任务回顾：实际/计划、完成或未完成；可选表情 | 点过「未完成」的项在日结标出 |
| F-11c | 取消当前航班 | 执飞中可取消当晚全部航程，不统计 | 页面仅显示「今日航班取消」 |
| F-12 | 家长设置 | 固定执飞模块、窗口起止；重置今日后任务池只留当前固定模块 | `/settings` 手机可用 |
...
freeMinutesRemaining ≈ freeBlock.remainingBudgetMinutes（随联动扣减/提前补给/延长航程动态变化）
```

**进港规则**（详见 §4.1）：
- 到点：轻提醒，**不阻断**；孩子可继续当前航段
- 提前：释放整分钟**按顺序补给**后续待飞航段（含自由飞，不含水果经停；水果经停自身提前结束也会释放）
...
| 断点 | 宽度 | 策略 |
|------|------|------|
| 平板主场景 | ≥768px | 燃料盘吸顶；任务池网格；执飞大倒计时 |
| 手机 | <768px | 简化时间条；任务池与顺序页均纵向 |
...
interface EveningSession {
  ...
  overtimeDrainNextTargetId: string | null;  // 下一分钟将扣的联动航段
...
interface Block {
  ...
  remainingBudgetMinutes: number;  // 初始 = planned；超时按顺序扣减时递减
  activeBudgetMinutes: number | null; // 本段起飞时快照的可用时长
  ...
  extendUsed: boolean;           // 保留字段，延长航程改由 session.voyageExtended
  queueNote: string | null;
  markedIncomplete: boolean;     // 执飞中点「未完成」打的标签
  originalPlannedMinutes: number; // 排航程时的计划分钟
}
```
...
**EveningSession 补充说明**：上表已含 `voyageExtended`、超时扣减游标与 checkpoint。P1 的 `correctionTags` 不进 MVP Block。
...
| 20:17–20:32 | 超时 15 min，按顺序共扣 15 分钟（各项各 5） | 10 | **15** | 10 | 25 |
| 20:32 | 数学进港 | — | — | — | 25 |
| 20:32–21:00 | 朗读/水果/订正仍可执飞（预算已缩短） | — | — | — | — |
| 21:30 | 日结：客观列出 incomplete 项 → 明早队列 | — | — | — | — |

**变体（部分超时）**：若数学超时 8 min 后进港，按顺序扣朗读 3 + 订正 3 + 自由飞 2 → 朗读剩 12、订正剩 12、自由飞剩 28。若至 21:30 仍够两项 minimum → 不弹优先级。若至 21:30 只剩 30 min → 孩子选先做订正 20 min → 剩 10 min → 系统预告朗读 incomplete。

**与旧模式差异**：拖沓代价体现在**后续航段按顺序少 1 分钟**直至 incomplete，全程**不打断做题**；仅在进港后时间 genuinely 不够时，**一次**让孩子选先做哪项。
...
| v0.3 | 2026-08-31 | 对照实现修订：朗读/水果不可移除；去掉秒级倒计时与「并行扣减」残留；修正 8 分钟超时示例 |
